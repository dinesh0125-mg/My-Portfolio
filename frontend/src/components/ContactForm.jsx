...
import { Send, Copy, Check, Mail, AlertCircle, Loader2 } from 'lucide-react';
...
export default function ContactForm() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      // 1. Submit message to backend MySQL database
      ...
      // Fallback: Open mailto directly if backend is unavailable
  ...
  return (
    <div className="bg-white rounded-2xl sm:rounded-3xl p-4 sm:p-8 border border-slate-200 shadow-card">
        ...
        <div className="mb-5 p-3.5 sm:p-4 rounded-2xl bg-teal-50 border border-teal-200 text-xs text-teal-800 flex items-start gap-2.5 animate-in fade-in duration-200">
          ...
          <div>
            ...
            <p className="text-teal-700 mt-0.5 break-all">
              I have received your message in my dashboard and will get back to you shortly at{' '}
      ...
      <form onSubmit={handleSubmit} noValidate className="space-y-4">
        {/* Name */}
        <div>
          <label htmlFor="name" className="block text-[11px] sm:text-xs font-bold text-slate-700 uppercase tracking-wider mb-1.5">
          ...
          <input
            ...
            className={`w-full text-sm px-3.5 sm:px-4 py-2.5 sm:py-3 rounded-xl border bg-slate-50/50 text-slate-900 placeholder:text-slate-400 focus:bg-white focus:outline-none focus:ring-2 focus:ring-teal-500/40 transition-all ${
              errors.name ? 'border-red-400 bg-red-50/30' : 'border-slate-200 focus:border-teal-500'
            }`}
          />
        ...
        {/* Email */}
        <div>
          <label htmlFor="email" className="block text-[11px] sm:text-xs font-bold text-slate-700 uppercase tracking-wider mb-1.5">
          ...
          <input
            ...
            className={`w-full text-sm px-3.5 sm:px-4 py-2.5 sm:py-3 rounded-xl border bg-slate-50/50 text-slate-900 placeholder:text-slate-400 focus:bg-white focus:outline-none focus:ring-2 focus:ring-teal-500/40 transition-all ${
              errors.email ? 'border-red-400 bg-red-50/30' : 'border-slate-200 focus:border-teal-500'
            }`}
          />
        ...
        {/* Message */}
        <div>
          <label htmlFor="message" className="block text-[11px] sm:text-xs font-bold text-slate-700 uppercase tracking-wider mb-1.5">
          ...
          <textarea
            ...
            className={`w-full text-sm px-3.5 sm:px-4 py-2.5 sm:py-3 rounded-xl border bg-slate-50/50 text-slate-900 placeholder:text-slate-400 focus:bg-white focus:outline-none focus:ring-2 focus:ring-teal-500/40 transition-all resize-none ${
              errors.message ? 'border-red-400 bg-red-50/30' : 'border-slate-200 focus:border-teal-500'
            }`}
          ></textarea>
